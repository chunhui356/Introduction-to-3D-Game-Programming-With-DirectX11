...
ID3D11ShaderResourceView* d3dHelper::CreateTexture2DArraySRV(
		...
		UINT mipFilter)
{
    ID3D11Texture2D * pTexute2D;
    ID3D11ShaderResourceView * pSRV;
    CreateDDSTextureFromFile(device, context, filenames[0].c_str(), (ID3D11Resource**)&pTexute2D, &pSRV);
...
void ExtractFrustumPlanes(XMFLOAT4 planes[6], CXMMATRIX Mat)
{
    MyFloat4x4 M(Mat);
	//
	// Left
